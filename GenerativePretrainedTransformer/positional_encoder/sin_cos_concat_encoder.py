...
class SinCosConcatTextEncoder(nn.Module):
    def __init__(self, n_tokens: int, d_model: int, init_range, device = device):
        ...
        # define the encoder
        self.encoder = nn.Embedding(n_tokens, d_model//2).to(device)
...
class SinCosConcatPosEncoder(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, max_seq_len: int = 5000, device = device):
        ...
        self.positional_encoding = torch.zeros(max_seq_len, d_model//2).to(device)
        positions = torch.arange(0, max_seq_len, dtype=torch.float).unsqueeze(1) / torch.pow(10000, 
            torch.arange(0, d_model//2, 2, dtype=torch.float) / (d_model))
