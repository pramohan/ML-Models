...
class WikiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.char_corruption:
            ...
            mask_len = random.randint(0, len(truncated_document)//2)
            prefix_len = random.randint(0, len(truncated_document) - mask_len)
            
            prefix = truncated_document[:prefix_len]
            mask = truncated_document[prefix_len:prefix_len+mask_len]
            suffix = truncated_document[prefix_len+mask_len:]
            ...
            input_string = torch.tensor([self.stoi[s] for s in masked_string[:-1]], dtype=torch.long)
            output_string = torch.tensor([self.stoi[s] for s in masked_string[1:]], dtype=torch.long)
            return input_string, output_string
            ##########################################
